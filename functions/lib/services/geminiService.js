...
exports.generateSocialPost = exports.generateUniquePendantReviews = exports.generateSouthAfricanReviews = exports.generateProductMetadataFromImage = exports.suggestMarketingCopy = exports.generateProductDescription = void 0;
...
const localGenerateProductName = (categoryContext, seed) => {
...
const localExtractTags = (categoryContext) => {
    const cat = (categoryContext || '').toLowerCase();
    const tags = [];
    if (cat.includes('ring'))
        tags.push('Ring');
    if (cat.includes('neck') || cat.includes('pendant') || cat.includes('choker'))
        tags.push('Pendant');
    if (cat.includes('ear'))
        tags.push('Stud');
    if (cat.includes('bracelet'))
        tags.push('Bracelet');
    if (tags.length === 0)
        tags.push(pick(['Pendant', 'Ring', 'Stud', 'Dangle']));
    // Ensure material-like tags
    tags.push(pick(materials));
    // Limit and unique
    return Array.from(new Set(tags)).slice(0, 5);
};
const localGenerateDescription = (name, category, keywords) => {
...
const localGenerateSeoKeywords = (name, tags) => {
...
const suggestMarketingCopy = async (specialTitle, discount) => {
    const ai = getAI();
    if (!ai)
...
exports.suggestMarketingCopy = suggestMarketingCopy;
const generateProductMetadataFromImage = async (base64Image, categoryContext) => {
    const ai = getAI();
    ...
    try {
        ...
        try {
            ...
            const description = localGenerateDescription(name, categoryContext || 'Jewelry', 'Handmade, Unique');
            const seoKeywords = localGenerateSeoKeywords(name, tags);
            const colors = []; // cannot reliably detect colors locally
            return {
                name,
                description,
                whenAndHowToWear: 'Perfect for both everyday wear and special occasions. Layer with other pieces for a curated look.',
...
exports.generateProductMetadataFromImage = generateProductMetadataFromImage;
const generateSouthAfricanReviews = async (productName, count) => {
    const ai = getAI();
    if (!ai)
        return [];
    try {
      ...
      Output strictly as a JSON Array of objects with keys: userName, location, content, rating, date.`,
        ...
        // Add IDs manually
        return rawReviews.map((r) => (Object.assign(Object.assign({}, r), { id: `rev_ai_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` })));
    }
    catch (error) {
...
exports.generateSouthAfricanReviews = generateSouthAfricanReviews;
const generateUniquePendantReviews = async (count) => {
    const ai = getAI();
    if (!ai)
        return [];
    try {
      ...
      Output strictly as a JSON Array of objects with keys: userName, location, content, rating, date.`,
        ...
        if (!text)
            return [];
        const rawReviews = JSON.parse(text);
        return rawReviews.map((r) => (Object.assign(Object.assign({}, r), { id: `rev_unique_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` })));
    }
    catch (error) {
...
exports.generateUniquePendantReviews = generateUniquePendantReviews;
const generateSocialPost = async (productName, platform, price) => {
    const ai = getAI();
    if (!ai)
