...
const AffiliateProgram: React.FC = () => {
  const { user, affiliateLeaderboard, applyForAffiliate, buyAffiliateContent, joinAffiliateElite, register, login, getAllUsers } = useStore();
  ...
  const [formData, setFormData] = useState({
      name: user.name !== 'Guest' ? user.name.split(' ')[0] : '',
      surname: user.name !== 'Guest' ? user.name.split(' ').slice(1).join(' ') : '',
      email: user.email || '',
      dob: user.birthday || '',
      gender: '',
      reason: '',
  ...
  }, [user.affiliateStats, user.id, getAllUsers]);
  ...
  const termsSections = useMemo(() => [
    ...
    {
        title: "2. Definitions",
        content: `"Partner", "You", and "Your" refer to the individual or entity applying for or participating in the Program. "We", "Us", and "Our" refer to Spoil Me Vintage. "Commission" refers to the amount paid for a successful referral.`
    },
    {
        title: "3. Enrollment",
        content: `To begin the enrollment process, you must submit a complete Partnership Application. We will evaluate your application and notify you of your acceptance or rejection.`
    },
    {
        title: "4. Affiliate Links",
        content: `You will be issued a unique Affiliate URL or code. It is your responsibility to ensure that these links are correctly formatted and used on your site.`
    },
    {
        title: "5. Commissions",
        content: `You will earn commissions based on the tiered structure defined in your dashboard. Standard commissions range from 10% to 20%. Additionally, you earn a 1% override commission on sales generated by partners you directly recruit.`
    },
    {
        title: "6. Payment Terms",
        content: `Commissions are paid once a month. Payments are made via EFT to a South African bank account. Minimum payout balance is R200.`
    },
  ...
  let currentTierFloor = 0;
  let progressPercent = 0;

  if (stats.totalSalesCount < 50) {
      nextTierText = "11% Commission";
      nextTierTarget = 50;
      currentTierFloor = 0;
  } else if (stats.totalSalesCount < 100) {
      nextTierText = "15% Commission";
      nextTierTarget = 100;
      currentTierFloor = 50;
  } else if (stats.totalSalesCount < 500) {
      nextTierText = "20% Lifetime Commission";
      nextTierTarget = 500;
      currentTierFloor = 100;
  } else {
  ...
  if (stats.totalSalesCount >= 500) {
      progressPercent = 100;
  } else {
      progressPercent = ((stats.totalSalesCount - currentTierFloor) / (nextTierTarget - currentTierFloor)) * 100;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 space-y-16">
        ...
        <div className="relative rounded-3xl overflow-hidden bg-zinc-900 border border-pink-500/30 min-h-[400px] flex items-center">
            ...
            <div className="relative z-20 p-8 md:p-16 max-w-2xl space-y-6">
                ...
                <h1 className="font-cherry text-5xl md:text-7xl text-white leading-none drop-shadow-[0_0_15px_rgba(236,72,153,0.6)]">
                    Partner & <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-500">Earn Cash</span>
                </h1>
                <p className="text-gray-300 text-lg leading-relaxed">
                    Turn your passion for vintage into profit. Join our exclusive Partnership Program and earn up to <strong>20% Lifetime Commission</strong> on every sale you generate.
                </p>
        ...
        {/* ... [How it Works, Commission Tiers, Leaderboard same as before] ... */}
        {/* HOW IT WORKS & TIERS */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="bg-zinc-900/50 p-6 rounded-2xl border border-gray-800 text-center relative group hover:bg-zinc-900 transition-colors">
                ...
                <p className="text-gray-400 text-sm">We payout commissions monthly via EFT to your South African bank account.</p>
            </div>
        </div>
            ...
            <div id="affiliate-dashboard" className="space-y-8 animate-in slide-in-from-bottom-10 duration-700 pt-8 border-t border-gray-800">
                <div className="flex flex-col md:flex-row justify-between items-end gap-4">
                    <div>
                        ...
                        <p className="text-gray-400">Welcome back, {user.name.split(' ')[0]}! Manage your links and earnings.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-zinc-900 p-6 rounded-2xl border border-gray-800">
                        ...
                        <div className="text-xs text-purple-400 mt-1">Tier: {stats.commissionRate >= 20 ? 'Gold Elite' : stats.commissionRate >= 15 ? 'Silver' : stats.commissionRate >= 11 ? 'Bronze' : 'Starter'}</div>
                    </div>
                </div>
